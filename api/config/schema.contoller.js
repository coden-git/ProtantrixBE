...
*/
    ...
    // previously we wrote the JSON to disk; now return it directly
    return projectJson;
}

// Express handler: run createActivityData and save the generated JSON into the
// `config` collection using the Config model with name 'json_scehma'.
// Usage: require this file and mount `updateConfigWithActivity` on a route.
const JsonSchema = require('./json_schema.model');

// Save generated activity JSON into `json_schema` collection as a new
// versioned document. Version counter is stored in `config.name = 'json_schema_version'`.
const Config = require('./config.model');

module.exports.updateConfigWithActivity = async function (req, res) {
    try {
        // createActivityData now returns the JSON directly
        const json = await Promise.resolve(createActivityData());

        // Read current counter value (do NOT increment yet)
        const currentCounter = await Config.findOne({ name: 'json_schema_version' }).lean();
        const currentValue = (currentCounter && typeof currentCounter.value === 'number') ? currentCounter.value : 0;
        const newVersion = currentValue + 1;

        // Create a new versioned document in json_schema collection using the
        // computed newVersion. Only if this creation succeeds will we update
        // the counter in the config document.
        const saved = await JsonSchema.create({
            version: newVersion,
            schema: json.activities, // store the full generated JSON
        });

        // Update the config counter to reflect the new version. Use $max so
        // we don't accidentally lower the counter if another process advanced
        // it concurrently. We intentionally update AFTER successful create so
        // the counter isn't incremented on failed saves.
        await Config.findOneAndUpdate(
            { name: 'json_schema_version' },
            { $max: { value: newVersion } },
            { new: true, upsert: true, setDefaultsOnInsert: true }
        );

        return res.status(200).json({ ok: true });
    } catch (err) {
        console.error('updateConfigWithActivity error:', err);
